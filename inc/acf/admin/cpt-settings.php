...
add_action("acf/post_type/basic_settings", function ($acf_post_type) {
	$fields = [
		"allow_document_upload" => [
			...
			"instructions" =>
				"Allow the editor the ability to add documents to the post type.",
		],
		"post_summary" => [
			"label" => "Summary box",
		...
		"revision_date" => [
			"label" => "Revision Date",
			"instructions" =>
				"Adds a revision date that can be used instead of the updated date",
		],
	];
	...
	foreach ($fields as $name => $data) {
		acf_render_field_wrap([
			...
			"value" => isset($acf_post_type[$name])
				? $acf_post_type[$name]
				: false,
			"type" => "true_false",
			"ui" => 1,
