...
add_filter(
	...
	function ($markup, $args, $breadcrumbs_instance) {
		...
		$markup =
			'<div class="breadcrumb_desktop wb-desktop-only">' .
			$markup .
			"</div>";
		...
		} elseif (is_single() && "page" != $post->post_type) {
			//Custom set parent page for flexible cpt
			$parent_page_id = govwind_get_post_type_setting(
	...
	<path d="M6.49833 10.5417L10.766 14.8094L10 15.5833L4.41667 10L10 4.41666L10.766 5.19062L6.49833 9.45833H15.5833V10.5417H6.49833Z" />
		...
		$mobile_link =
			...
			esc_url($mobile_url) .
			'">' .
			esc_html($mobile_title) .
			"</a>";
		$markup =
			$markup .
			'<div class="breadcrumb_mobile wb-mobile-only">' .
			$back_svg .
			$mobile_link .
			"</div>";
		return $markup;
